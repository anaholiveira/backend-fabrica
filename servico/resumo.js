...
export async function listarPedidosAdmin(req, res) {
  try {
    ...
    for (const row of rows) {
      ...
      if (row.tipo === 'recheio') cupcake.recheio = row.nome_ingrediente;
      if (row.tipo === 'cobertura') cupcake.cobertura = row.nome_ingrediente;
      if (row.tipo === 'cor_cobertura') cupcake.cor_cobertura = row.nome_ingrediente;
    }
